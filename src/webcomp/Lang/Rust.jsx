...
const Rust = () => {
  ...
  useEffect(() => {
    fetch('/Lang/Rust.json') // Ensure to provide the correct path to your JSON file
    ...
    <>
      ...
      <div className="w-full bg-gradient-to-r from-purple-100 via-purple-200 to-purple-300 text-gray-800">
        <div className="p-6 max-w-4xl mx-auto">
          <h1 className="text-3xl font-bold mb-6 text-teal-400">Rust Concepts</h1>
                    ...
                    <ul className="list-disc pl-5 text-gray-300 space-y-2">
                      {Object.keys(data[key].examples).map((exampleKey) => (
                        <li key={exampleKey} className="flex flex-col space-y-1">
                          <strong className="text-gray-200">{exampleKey.charAt(0).toUpperCase() + exampleKey.slice(1)}:</strong> 
                          <code className="bg-gray-700 p-1 rounded text-gray-100">
                            {data[key].examples[exampleKey].rust}
                          </code>
                        </li>
                      ))}
                    </ul>
